...
- `ci_method` (`"cp" | "normal"`): method for the **permutation p-value CI** in config.

Note on naming (*known issue to be fixed*):

- The *config key* `ci_method` uses `"cp"` or `"normal"`.
- The *public `ritest(...)` argument* annotation lists `"clopper-pearson"` or `"normal"`. 
